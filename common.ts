...
import { readableStreamToString, ZapierEmbedError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://zapier.com/oauth/authorize/"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new ZapierEmbedError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new ZapierEmbedError(e, e.response?.data)
                }
                if (innerError instanceof ZapierEmbedError) {
                    // Got "ZapierEmbedError" from the above try block
                    throw innerError;
                }
